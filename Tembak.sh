#!/system/xbin/bash
#Bersihkan Layar
clear
blue='\e[1;34m'
green='\e[1;32m'
purple='\[1;35m'
cyan='\e[1;36m'
red='\e[1;31m'
white='\e[1;37m'
yellow='\e[1;33m'
sleep 1

echo "=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*"
toilet -f standard -F gay "FreeKuota"
echo "=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*"
echo ""
echo "Author : " Mr.®evoer"
" Team : " Moeslim Cyber Attack X JCA Attacker"
echo " Kontak Me : " 0857xxxxx"
" Pesan : "Jangan Di Salah Gunakan Dan Disebar :) Karena Nanti Dosa Tanggung Sendiri:)"
sleep 1
echo "ThanksTo:"
echo "Allmember  Cyber Attacker"
echo "=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*"
echo ""
sleep 2
echo "Mau Tembak Kuota...""Silahkan Pilih No Dibawah Bacaan Ini !"
sleep 0.3
echo "1.XL/AXIS" a;
sleep 0.3
echo "2.INDOSAT" b;
sleep 0.3
echo "3.SIMPATI" c;
sleep 0.3
echo "4.SMARTFREN" d; 
sleep 0.3
echo "5.3(Three)" e;
sleep 0.3
echo "masukkan kodenya"
read p;

echo "Sedang Menuju Proses..."
sleep 3
echo "█10%"
sleep 2
echo "██20%"
sleep 5
echo "███30%"
sleep 3
echo "████40%"
sleep 3
echo "█████50%"
sleep 3
echo "██████60%"
sleep 3
echo "███████70%"
sleep 4
echo "████████80%"
sleep 3
echo "█████████90%"
sleep 4
echo "██████████100%"
sleep 3
clear

echo "Masukkan Nomer Anda Yang Benar...Contoh:628xxx"
sleep 2
echo "Isi Dibawah!"
read p;
clear

echo "Silahkan Pilih Dari Daftar Menu Dibawah Ini!"
sleep 2
echo "1.Paket UNLIMITED INDOSAT"
sleep 2
echo "2.Paket UNLIMITED SMARTFREN"
sleep 2
echo "3.Paket UNLIMITED AXIS/XL"
sleep 2
echo "4.Paket UNLIMITED SIMPATI/TELKOMSEL"
sleep 2
echo "5.Paket UNLIMITED 3(Three)"
sleep 2
echo "masukkan nomornya..."
read p;
clear
echo "Harap Masukan Informasi Ulang.."
from multiprocessing.pool import ThreadPool
try:
	import os, random
	from time import sleep as turu
	import subprocess as sp
	import requests
except ModuleNotFoundError:
	print("\nSepertinya module requests BELUM Di Install")
	print("$ pip install requests\n")
	exit()

try:
	os.system('clear')
	print("""\033[1;32m
   _  _     \033[1;36mTembak Kuita Free All Oprator\033[1;32m
 _| || |_   \033[1;31mAuthor : Mr.®evoers\033[1;32m
|_  ..  _|  \033[1;31mContact : 085736891082\033[1;32m
|_      _|  \033[1;31mgithub : https://github.com/WhatTheSemvak\033[1;32m
  |_||_| 
""")
	no = input("\033[1;37m[?] NOMOR (Pakai 62 Gan) =>\033[1;36m ")
  read -p "Kartu : "
	jum=int(input("\033[1;37m[?] Jumlah Kuota(JIKA 2GB Tulis Angka 2 Dan Seterusnya) => \033[1;36m"))
except KeyboardInterrupt:
	print("\nKey interrupt")
	exit()
print()
print("[*] RESULT:")
def main(arg):
	try:
		idk=('phoneNumber')
		r = requests.post('https://p.grabtaxi.com/api/passenger/v2/profiles/register', data={'phoneNumber':no, 'countryCode': 'ID', 'name': 'nuubi', 'email': 'nuubi@mail.com', 'deviceToken': '*'}, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.117 Safari/537.36'})
#		print(r.text)
		if str(idk) in str(r.text):
			print("\033[1;32m[+] SUKSES Suntik")
		else:
			print("\033[1;31m[-] GAGAL Suntik")
	except: pass

jobs = []
for x in range(jum):
    jobs.append(x)
p=ThreadPool(5)
p.map(main,jobs)
print("Tunggu Sekitar 1 Menit")


clear
echo "\033[33;1mTunggulah Sekitar 1 Menit Untuk Mendapatkan Sms Masuk...."
sleep 1
echo "Jika 1Menit Belum Ada Sms Masuk Berarti No Kamu tidak Valid...!!"
sleep 2
echo "Thanks To:Ivan Cruzz Revoer - Mr.®evoer- Moeslim Cyber Attack"
sleep 2
echo "Pesan:Jangan Salahin Admin Ya Kentod...Mau Kuota Gratis Ga Sabar..."
sleep 3
